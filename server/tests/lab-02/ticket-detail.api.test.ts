...
describe("GET /api/tickets/:id (Issue 9 - Ticket Detail & Ownership)", () => {
  ...
  beforeAll(async () => {
    ...
    if (!a) a = await prisma.user.create({ data: { name: "Issue9 Owner", email: OWNER_EMAIL, passwordHash: "dummy", isActive: true } });
    owner = a;

    let b = await prisma.user.findUnique({ where: { email: OTHER_EMAIL } });
    if (!b) b = await prisma.user.create({ data: { name: "Issue9 Other", email: OTHER_EMAIL, passwordHash: "dummy", isActive: true } });
    other = b;
