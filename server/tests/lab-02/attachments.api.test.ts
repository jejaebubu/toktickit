...
describe("Attachment API (Issue 9 - Upload / Download / Soft-Remove)", () => {
  ...
  beforeAll(async () => {
    ...
    if (!a) a = await prisma.user.create({ data: { name: "Attachment Owner", email: OWNER_EMAIL, passwordHash: "dummy", role: "REQUESTER", isActive: true } });
    owner = a;

    let b = await prisma.user.findUnique({ where: { email: OTHER_EMAIL } });
    if (!b) b = await prisma.user.create({ data: { name: "Attachment Other", email: OTHER_EMAIL, passwordHash: "dummy", role: "REQUESTER", isActive: true } });
    other = b;
